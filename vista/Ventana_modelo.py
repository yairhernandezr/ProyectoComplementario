...
class Ventana_Modelo(tk.Toplevel):
    """Clase Ventana_Modelo """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        #****************************************Configuracion Ventana **************************
        self.config(background="black")
        self.title("Ventana Modelo")
        self.geometry("365x200")
         
        #****************************************** Frame Label***********************************
        self.lframe_modelo = LabelFrame(self,text="Modelo",font=("Roboto Cn",10),bg="black",fg="white")
        self.lframe_modelo.place(x= 5, y=10, width= 350)
        #******************************************Frame Botones************************************
        ...
        #****************************************label *******************************************
        self.lmodelo=Label(self.lframe_modelo,text="Nombre Modelo",font=("Roboto Cn",12),bg="black",fg="white")
        self.lmodelo.grid(row=0,column=0,padx=10,pady=10)
        #*****************************************Entrada *****************************************
        self.emodelo=Entry(self.lframe_modelo,font=("Roboto Cn",12),bg="black",fg="white",bd=2)
        self.emodelo.grid(row=0,column=1)
        ...
        #**************************************** Botones ****************************************
        self.bguardar=Button(self.lframe_boton,image=self.imguardar,command=self.guardarmodelo)
    ...
    def guardarmodelo(self):
        if self.emodelo.get()==" ":
            messagebox.showinfo(message="¿Digite el nombre de la Marca porfavor?", title="validar Modelo")
            self.emodelo.focus_set()
        else:
            m=messagebox.askquestion(message="¿Desea Guardar el registro?", title="Guardar")
            if m=="yes":
                modelo=Modelos(self.emodelo.get(),)
                INSERTAR='INSERT INTO modelo VALUES(null,%s)'
                dato=[modelo.modelos]
                Modelo_D.insertar(INSERTAR,dato)
